        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('Volunteer_payment.update', $volenteerPaymentResult->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                            ...
                            <select id="volunteer_id" name="volunteer_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                ...
                                @foreach($userResult as $userData)
                                    @if(Auth::user()->type == 'admin' || Auth::user()->id == $userData->id)
                                        <option value="{{ $userData->id }}" {{ $volenteerPaymentResult->volunteer_id == $userData->id ? 'Selected' : '' }}> {{ $userData->name }} </option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="number" name="amount" id="amount" value="{{ $volenteerPaymentResult->amount }}" class="border rounded-md w-full py-2 px-3 text-gray-700">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="remarks" id="remarks" value="{{ $volenteerPaymentResult->remarks }}" class="border rounded-md w-full py-2 px-3 text-gray-700">
                        </div>
